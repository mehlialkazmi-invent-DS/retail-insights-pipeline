...
| `dc_daily`, `dc_inst` | `(product_id, warehouse_id)` — same grain, but **one independent same-pairs universe each**: `dc_daily`'s from `dc_daily`, `dc_inst`'s from `dc_inst` (see [`dc_instock`](#dc_instock)) |
